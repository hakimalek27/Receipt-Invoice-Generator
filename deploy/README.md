...
## Deploying `feature/wehdah-ws-spec-redesign` to production

If the production dashboard (`resit.wehdah.my`) still shows the old
Master Data layout (only Company/Customers/Products/Templates/Numbering
tabs, no Branding / Bank Accounts / PDF Boilerplate) AND PDFs render
with the generic Malay template ("JUMLAH BESAR", "Kepada / Bill To"),
the branch hasn't been deployed yet. Apply this runbook:

```bash
# On the production server (43.133.34.55 or wherever):
cd /var/www/receipt-invoice-generator

# 1) Pull the feature branch (or the merge commit on main).
sudo -u www-data git fetch origin
sudo -u www-data git checkout feature/wehdah-ws-spec-redesign
sudo -u www-data git pull --ff-only origin feature/wehdah-ws-spec-redesign

# 2) Install PHP deps (no new packages but lock file may have updated).
sudo -u www-data composer install --no-dev --optimize-autoloader --no-interaction

# 3) Apply migrations (Phase 2A + 2B + boilerplate columns are additive,
#    all nullable, safe to roll forward).
sudo -u www-data php artisan migrate --force

# 4) Refresh seeded brand palettes (VD orange/teal, NCS navy/cream/gold).
#    Skip if production already has live data — only re-seed companies
#    you don't mind overwriting.
# sudo -u www-data php artisan db:seed --class=CompanySeeder --force

# 5) Build frontend assets (Vite -> public/build).
#    Run this on a machine with Node 20+; copy public/build/ to prod.
npm ci
npm run build

# 6) Clear & cache config/routes/views.
...
# 7) Storage symlink (if not already done).
sudo -u www-data php artisan storage:link

# 8) Reload services so the new public/.user.ini PHP limits + queue
#    listeners pick up.
sudo systemctl reload php8.3-fpm
sudo systemctl reload nginx
sudo supervisorctl restart rig-queue:*
...
# 9) Smoke check.
curl -I https://resit.wehdah.my  # expect 200 + HSTS
curl https://resit.wehdah.my/login  # expect login page
# Login + visit /master-data: should now show 8 tabs (Company, Branding,
# Bank Accounts, PDF Boilerplate, Customers, Products, Templates, Numbering).
# Visit /documents/create + Issue + Preview A4: PDF should render with the
# dark blue "INVOICE" title strip (NOT the old "JUMLAH BESAR" generic
# template).
```

### Verifying the right templates are loaded

After deploy, the `tests/Feature/FullStackSmokeTest.php` suite gates this.
Run on prod:

```bash
sudo -u www-data php artisan test --filter='FullStackSmokeTest'
```

All 16 cases must pass:
- Master Data Inertia page returns all tabs.
- Company name/address/phone/email can be updated.
- Brand colors persist.
- Logo/stamp/signature upload + delete.
- Bank accounts CRUD.
- PDF boilerplate per-doc-type persists.
- Customer + Product + Template + Numbering CRUD.
- Inertia shares active_company + available_companies.
- Documents/Index filter chain works.
- **`pdf.wehdah.invoice` template (not generic) is resolved for WS company.**

If the last test fails, the company seeded on prod has `code != 'WS'` —
update via `php artisan tinker` (`Company::first()->update(['code' => 'WS'])`).
