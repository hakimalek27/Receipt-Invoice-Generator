...
<head>
    <meta charset="utf-8">
    <title>{{ $document->official_number ?? 'PAYMENT VOUCHER' }}</title>
...
@foreach($itemPages as $pageIndex => $pageItems)
    ...
    @if($pageIndex === 0)
        @include('pdf.wehdah._header', ['variant' => 'full', 'documentTitle' => 'PAYMENT VOUCHER'])
        <div class="ws-meta-row">
            <div class="ws-meta-col ws-meta-col-left">
                @include('pdf.wehdah._bill-to', ['label' => 'Paid To:'])
            </div>
            <div class="ws-meta-col ws-meta-col-right">
                @include('pdf.wehdah._meta-block', ['documentTitleEn' => 'PAYMENT VOUCHER', 'showValidity' => false])
            </div>
        </div>
        ...
        @endif
    @else
        @include('pdf.wehdah._header', ['variant' => 'compact', 'documentTitle' => 'PAYMENT VOUCHER'])
    @endif
    ...
    @else
        <div class="ws-totals-row">
            ...
            <div class="ws-totals-grand-cell">
                <table class="ws-grand-table">
                    <tr>
                        <td class="ws-grand-label">Total Paid ({{ $document->currency }})</td>
        ...
        {{-- Bank box intentionally omitted: PV records an outgoing payment;
             there is no inbound bank context to display. --}}
        ...
        @include('pdf.wehdah._signature', [
            'leftIntro' => $boilerplate['signature_left_intro'] ?? 'Prepared by,',
            'leftLabel' => $boilerplate['signature_left_label'] ?? 'Prepared by',
            'rightIntro' => $boilerplate['signature_right_intro'] ?? 'Approved by,',
            'rightLabel' => $boilerplate['signature_right_label'] ?? 'Approved by',
        ])
    @endif
