...
<head>
    <meta charset="utf-8">
    <title>{{ $document->official_number ?? 'CASH BILL' }}</title>
...
@foreach($itemPages as $pageIndex => $pageItems)
    ...
    @if($pageIndex === 0)
        @include('pdf.wehdah._header', ['variant' => 'full', 'documentTitle' => 'CASH BILL'])
        <div class="ws-meta-row">
            ...
            <div class="ws-meta-col ws-meta-col-right">
                @include('pdf.wehdah._meta-block', ['documentTitleEn' => 'CASH BILL', 'showValidity' => false])
            </div>
        </div>
        ...
        @endif
    @else
        @include('pdf.wehdah._header', ['variant' => 'compact', 'documentTitle' => 'CASH BILL'])
    @endif
    ...
    @else
        ...
        @include('pdf.wehdah._signature', [
            ...
            'rightIntro' => $boilerplate['signature_right_intro'] ?? 'Goods received & payment settled in full',
...
@include('pdf.partials.artwork-pages', ['documentTitleEn' => 'CASH BILL', 'showConfirmation' => true])
</body>
</html>
