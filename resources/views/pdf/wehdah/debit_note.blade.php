...
<head>
    <meta charset="utf-8">
    <title>{{ $document->official_number ?? 'DEBIT NOTE' }}</title>
...
@foreach($itemPages as $pageIndex => $pageItems)
    ...
    @if($pageIndex === 0)
        @include('pdf.wehdah._header', ['variant' => 'full', 'documentTitle' => 'DEBIT NOTE'])
        <div class="ws-meta-row">
            <div class="ws-meta-col ws-meta-col-left">
                @include('pdf.wehdah._bill-to', ['label' => 'Debit To:'])
            </div>
            <div class="ws-meta-col ws-meta-col-right">
                @include('pdf.wehdah._meta-block', ['documentTitleEn' => 'DEBIT NOTE', 'showValidity' => false])
            </div>
        </div>
        ...
        @endif
    @else
        @include('pdf.wehdah._header', ['variant' => 'compact', 'documentTitle' => 'DEBIT NOTE'])
    @endif
    ...
    <table class="ws-items">
        <thead>
            <tr>
                ...
                <th class="ws-col-total">Debit Amount</th>
            </tr>
        </thead>
        <tbody>
    ...
    @else
        <div class="ws-totals-row">
            ...
            <div class="ws-totals-grand-cell">
                ...
                <table class="ws-grand-table">
                    <tr>
                        <td class="ws-grand-label">Total Debit ({{ $document->currency }})</td>
        ...
        @include('pdf.wehdah._signature', [
            'leftIntro' => $boilerplate['signature_left_intro'] ?? 'Issued by,',
            'leftLabel' => $boilerplate['signature_left_label'] ?? 'Authorised Signature',
            'rightIntro' => $boilerplate['signature_right_intro'] ?? 'Additional charge acknowledged by',
...
@include('pdf.partials.artwork-pages', ['documentTitleEn' => 'DEBIT NOTE', 'showConfirmation' => true])
</body>
</html>
