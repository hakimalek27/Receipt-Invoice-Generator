...
<head>
    <meta charset="utf-8">
    <title>{{ $document->official_number ?? 'PROFORMA INVOICE' }}</title>
...
@foreach($itemPages as $pageIndex => $pageItems)
    ...
    @if($pageIndex === 0)
        @include('pdf.wehdah._header', ['variant' => 'full', 'documentTitle' => 'PROFORMA INVOICE'])
        <div class="ws-meta-row">
            ...
            <div class="ws-meta-col ws-meta-col-right">
                @include('pdf.wehdah._meta-block', ['documentTitleEn' => 'PROFORMA INVOICE', 'showValidity' => true])
            </div>
        </div>
        ...
        @endif
    @else
        @include('pdf.wehdah._header', ['variant' => 'compact', 'documentTitle' => 'PROFORMA INVOICE'])
    @endif
    ...
    @else
        ...
        @include('pdf.wehdah._signature', [
            ...
            'rightIntro' => $boilerplate['signature_right_intro'] ?? 'Acknowledged & accepted by',
...
@include('pdf.partials.artwork-pages', ['documentTitleEn' => 'PROFORMA INVOICE', 'showConfirmation' => true])
</body>
</html>
