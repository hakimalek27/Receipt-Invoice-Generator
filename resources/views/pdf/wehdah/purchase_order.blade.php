...
<head>
    <meta charset="utf-8">
    <title>{{ $document->official_number ?? 'PURCHASE ORDER' }}</title>
...
@foreach($itemPages as $pageIndex => $pageItems)
    ...
    @if($pageIndex === 0)
        @include('pdf.wehdah._header', ['variant' => 'full', 'documentTitle' => 'PURCHASE ORDER'])
        <div class="ws-meta-row">
            <div class="ws-meta-col ws-meta-col-left">
                @include('pdf.wehdah._bill-to', ['label' => 'Vendor:'])
            </div>
            <div class="ws-meta-col ws-meta-col-right">
                @include('pdf.wehdah._meta-block', ['documentTitleEn' => 'PURCHASE ORDER', 'showValidity' => false])
            </div>
        </div>
        ...
        @endif
    @else
        @include('pdf.wehdah._header', ['variant' => 'compact', 'documentTitle' => 'PURCHASE ORDER'])
    @endif
    ...
    @else
        <div class="ws-totals-row">
            ...
            <div class="ws-totals-grand-cell">
                ...
                <table class="ws-grand-table">
                    <tr>
                        <td class="ws-grand-label">Order Total ({{ $document->currency }})</td>
        ...
        {{-- Bank box intentionally omitted: PO goes OUT to a vendor; the
             vendor doesn't need our bank account details. --}}
        ...
        @include('pdf.wehdah._signature', [
            'leftIntro' => $boilerplate['signature_left_intro'] ?? 'Authorised by,',
            'leftLabel' => $boilerplate['signature_left_label'] ?? 'Authorised Signature',
            'rightIntro' => $boilerplate['signature_right_intro'] ?? 'Vendor acceptance',
            'rightLabel' => $boilerplate['signature_right_label'] ?? 'Vendor Sign & Chop',
        ])
    @endif
...
@include('pdf.partials.artwork-pages', ['documentTitleEn' => 'PURCHASE ORDER', 'showConfirmation' => true])
</body>
</html>
