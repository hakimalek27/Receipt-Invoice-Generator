...
<head>
    ...
    <style>
        ...
        .wehdah-items th { color: #fff; }
        .vd-payment-meta { margin: 8px 0; padding: 6px 10px;
                           background: {{ $brand['secondary'] ?? '#FBEEE6' }};
                           border: 1px solid {{ $brand['accent'] ?? '#16A085' }};
                           font-size: 8.5pt; }
        .vd-payment-meta strong { color: {{ $brand['primary'] ?? '#E67E22' }}; }
        .vd-payment-meta-row { display: table; width: 100%; }
        .vd-payment-meta-cell { display: table-cell; padding-right: 18px; }
    </style>
</head>
<body>
    ...
    @else
        ...
        @if(!empty($payment))
            <div class="vd-payment-meta">
                <div class="vd-payment-meta-row">
                    @if(!empty($payment['method']))
                        <div class="vd-payment-meta-cell"><strong>Method:</strong> {{ str_replace('_', ' ', ucfirst($payment['method'])) }}</div>
                    @endif
                    @if(!empty($payment['reference_number']))
                        <div class="vd-payment-meta-cell"><strong>Reference:</strong> {{ $payment['reference_number'] }}</div>
                    @endif
                    @if(!empty($payment['payment_date']))
                        <div class="vd-payment-meta-cell"><strong>Payment Date:</strong> {{ $payment['payment_date'] }}</div>
                    @endif
                </div>
            </div>
        @endif
        ...
        <div class="wehdah-terms">
            All beauty and lifestyle services are non-refundable once rendered.<br>
